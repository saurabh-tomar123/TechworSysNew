      ...
      <section className="bg-white py-16 px-6 md:px-16">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((item, index) => (
            <div
              key={index}
              className="group bg-gray-100 rounded-lg p-6 flex items-start gap-4 shadow-sm hover:shadow-md transition hover:bg-[#3D454C] "
            >
              <img src={iconImage} alt="Feature Icon" className="w-10 h-10" />
              <div>
                <h3 className="text-lg font-semibold text-black mb-1 group-hover:text-white">{item.title}</h3>
                <p className="text-gray-600 text-sm group-hover:text-white">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        bg-custom-gradient border-grayBorder text-white rounded-xl px-6 md:px-12 py-10 relative border border-gray-700">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
            ...
            <div className="flex items-center md:items-end gap-3">
              ...
              <button className="bg-yellow-300 hover:bg-yellow-400 text-black font-medium px-6 py-2 rounded-full transition-all duration-300 transform hover:shadow-[0_0_20px_rgba(250,204,21,0.7)]" onClick={() => navigate('contactus')}>
                Contact us
              </button>
            </div>
          </div>
            ...
            <div className="bg-[#1f1f1f] min-w-[180px] flex flex-col">
              {industries.map((item, idx) => (
                <button
                  ...
                  className={`text-left px-6 py-4 text-sm font-medium ${activeTab === item ? 'bg-yellow-400 text-black' : 'text-white hover:bg-[#333]'
                    } transition`}
                >
                  {item}
                </button>
          ...
          <div className="flex gap-[10px] flex-wrap sm:flex-nowrap ">
            ...
            <div className="bg-black text-white rounded-xl p-6 flex flex-col justify-between min-h-[320px]">
              ...
              <div>
                ...
                {services[0].quote && (
                  <button className="group bg-white text-black px-5 py-2 rounded-full text-sm font-medium inline-flex items-center gap-1 hover:bg-yellow-400 transition-colors duration-300">
                    Get a Quote <img src={btnArrow} className='h-[17px] w-[18px] transition-transform duration-500 ease-in-out group-hover:rotate-45' />
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="bg-black text-white rounded-xl p-6 flex justify-between ">
                <div className='text-left flex flex-col justify-between '>
                  ...
                  <div>
                    <button className="group bg-white text-black mt-4 px-4 py-1.5 rounded-full text-sm font-medium inline-flex items-center gap-1 hover:bg-yellow-400 transition-colors duration-300">
                      Get a Quote <img src={btnArrow} className='h-[17px] w-[18px] transition-transform duration-500 ease-in-out group-hover:rotate-45' />
                    </button>
                  </div>
                </div>
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-10 max-w-7xl mx-auto">
          ...
          <div className="z-10 relative top-[28%] ">
            ...
            <p className=" text-[30px] leading-[30px] w-[60%] max-w-[700px] text-lg mt-6 text-gray-300">
              For Startups & SMEs. Business-results oriented and AI-powered 🚀
            </p>
            <button className="group bg-white w-[186px] h-[60px] pl-[2.6rem] text-black mt-4 px-4 py-1.5 rounded-full text-sm font-medium inline-flex items-center gap-1 hover:bg-yellow-400 transition-colors duration-300">
              Get a Quote
              <img
                src={btnArrow}
                className="h-[17px] w-[18px] transition-transform duration-500 ease-in-out group-hover:rotate-45"
                alt="arrow"
              />
            </button>
